...
# Lock to prevent ELM327 communications occuring when an existing one
# still running.
...
def ConnectELM327(ThisDisplay):
    ...
    try:
        ...
        ThisDisplay.SetVisualText(
            ThisDisplay.ELM327Info,
            "INFO",
            "CONNECTING TO CAN BUS FOR OBDII COMMUNICATION...\n",
            False)
...
def FreezeFrameData(ThisDisplay):
    try:
        for FreezeIndex in range(ThisELM327.GetFreezeFrameCount()):
            ...
            # Get the information available for each of the supported PIDs.
            ThisDisplay.SetVisualText(
                ThisDisplay.FreezeFrameData, "INFO", "", False)
            for PID in sorted(ValidPIDs):
                PidData = ThisELM327.DoPID(PID[:4], FreezeIndex)
                ThisDisplay.SetVisualText(
                    ThisDisplay.FreezeFrameData,
                    "INFO",
                    "[" + PID + "] " + ValidPIDs[PID] + "\n",
                    True,
                    PidData)
...
def TroubleInfo(ThisDisplay):
    try:
        ...
        ThisDisplay.SetVisualText(ThisDisplay.TroubleInfo,
                                  "INFO",
                                  "\nSTORED TROUBLE CODES: [" + str(len(TroubleCodes)) + "]\n",
                                  True)
        if TroubleCodes != ELM327.STRING_ERROR:
            for TroubleCode in sorted(TroubleCodes):
                ThisDisplay.SetVisualText(ThisDisplay.TroubleInfo, "INFO", str(
                    TroubleCode) + " " + str(TroubleCodes[TroubleCode]) + "\n", True)
    ...
    try:
        ...
        for ThisGadgit in ThisDisplay.Meters:
            if type(ThisDisplay.Meters[ThisGadgit]) is Gadgit.Gadgit:
                ...
                if PID != "":
                    # Store the information returned for the current PID on the
                    # related meter.
                    PidData = ThisELM327.DoPID(PID)
...
def AquisitionLoop(ThisDisplay):
    try:
        while (ThisDisplay.Meters["GO_STOP"].GetDown()
               or ThisDisplay.Plots["GO_STOP"].GetDown()):
            # Update the gadgit data from the ECU.
            if ThisDisplay.CurrentTab == ThisDisplay.Meters and ThisDisplay.Meters["LOCK"].GetDown(
            ) and ThisDisplay.Meters["GO_STOP"].GetDown():
                if LockELM327.acquire(0):
                    _thread.start_new_thread(MeterData, (ThisDisplay, ))
            # Update the plot data from the ECU.
            if ThisDisplay.CurrentTab == ThisDisplay.Plots and ThisDisplay.Plots["GO_STOP"].GetDown(
            ):
                if LockELM327.acquire(0):
...
# Create a timer for updating the displayed time/date and updating gadgit
# data from the ECU.
...
while ExitFlag == False:
    ...
    for ThisEvent in pygame.event.get():
        ...
        if ThisEvent.type == pygame.QUIT:
            ...
            if ThisEvent.key == pygame.K_ESCAPE:
                # Close an open dialog if one exists.
                if "CONFIRM" in ThisDisplay.CurrentTab:
                    ThisDisplay.CurrentTab.pop("CONFIRM")
                elif "SELECT" in ThisDisplay.CurrentTab:
                    ThisDisplay.CurrentTab.pop("SELECT")
                elif "CONFIGURE" in ThisDisplay.CurrentTab:
                    ThisDisplay.CurrentTab.pop("CONFIGURE")
                else:
            ...
            try:
                ...
                NowDate = Now.strftime("%Y-%m-%d")
                ThisDisplay.SetVisualText(
                    ThisDisplay.CurrentTab, "TIME", NowTime)
                ThisDisplay.SetVisualText(
                    ThisDisplay.CurrentTab, "DATE", NowDate)
                ...
                print(str(Catch))
        # Only process the following events if the ELM327 device is currently
        # communicating.
        elif LockELM327.locked():
            if ThisEvent.type == pygame.MOUSEBUTTONDOWN:
                # Allow GO/STOP button to be toggled while ELM327
                # communications are occuring.
                if ThisDisplay.CurrentTab == ThisDisplay.Meters:
                    ThisDisplay.Meters["GO_STOP"].IsEvent(
                        Visual.EVENT_MOUSE_DOWN, ThisEvent.pos[0], ThisEvent.pos[1], ThisEvent.button)
                elif ThisDisplay.CurrentTab == ThisDisplay.Plots:
                    ThisDisplay.Plots["GO_STOP"].IsEvent(
                        Visual.EVENT_MOUSE_DOWN, ThisEvent.pos[0], ThisEvent.pos[1], ThisEvent.button)

        # Only process the following events if the ELM327 device is not
        # currently communicating.
        elif LockELM327.locked() == False:
            if ThisEvent.type == pygame.MOUSEBUTTONDOWN:
                # Pass button down events to all buttons and gadgits.
                ButtonGadgit = ThisDisplay.IsEvent(
                    Visual.EVENT_MOUSE_DOWN,
                    ThisEvent.pos[0],
                    ThisEvent.pos[1],
                    ThisEvent.button)
                ...
                if ButtonGadgit:
                    ...
                    if ButtonGadgit["BUTTON"] == "EXIT":
                        ...
                        ThisDisplay.CurrentTab["CONFIRM"] = Confirm.Confirm(
                            ThisDisplay.ThisSurface, "CONFIRM_EXIT", "Exit the application?")
                    # If confirm dialog button yes is pressed, close the
                    # dialog.
                    ...
                    # If select dialog selection is made, close the dialog.
                        ...
                        if ButtonGadgit["SELECTED"]:
                            SelectLines = SelectText.split('\n')
                            SelectedLine = SelectLines[ButtonGadgit["SELECTED"] - 1]
                            if ButtonGadgit["GADGIT"] == "SELECT_PID":
                                ThisPID = SelectedLine[SelectedLine.find(
                                    "[") + 1:SelectedLine.find("]")]
                                # Get a list of all valid PIDs the connected
                                # ECU supports.
                                ValidPIDs = ThisELM327.GetValidPIDs()
                                if ThisPID in ValidPIDs:
                                    if SelectGadgit[:5] != "PLOT_":
                                        ThisDisplay.Meters[SelectGadgit].SetPID(
                                            ThisPID, ValidPIDs[ThisPID])
                                    else:
                                        ThisDisplay.Plots["PLOT"].SetPID(
                                            int(SelectGadgit[5]) - 1, ThisPID, ValidPIDs[ThisPID])
                                else:
                                    if SelectGadgit[:5] != "PLOT_":
                                        ThisDisplay.Meters[SelectGadgit].SetPID(
                                            "", "")
                    ...
                    # If print button is pressed.
                        ...
                        ThisDisplay.CurrentTab["CONFIRM"] = Confirm.Confirm(
                            ThisDisplay.ThisSurface,
                            "CONFIRM_PRINT",
                            "OBDII Report Sent To Default Printer\n" + Result,
                            ThisDisplay.GetDisplayWidth() / 1.5,
                            True)
                    ...
                    # If save config button is pressed.
                        ...
                        ThisDisplay.CurrentTab["SELECT"] = Select.Select(
                            ThisDisplay.ThisSurface, "SELECT_FONT_NAME", SelectText)
                        ...
                        # Display a font name selection dialog.
                        ThisDisplay.CurrentTab["SELECT"] = Select.Select(
                            ThisDisplay.ThisSurface, "SELECT_VEHICLE_NAME", SelectText)
                        ...
                        # Display a font name selection dialog.
                        ThisDisplay.CurrentTab["SELECT"] = Select.Select(
                            ThisDisplay.ThisSurface, "SELECT_SERIAL_PORT_NAME", SelectText)
                    # If connect button is pressed, connect to the CAN BUS.
                    elif ButtonGadgit["BUTTON"] == "CONNECT":
                        if LockELM327.acquire(0):
                            _thread.start_new_thread(
                                ConnectELM327, (ThisDisplay, ))
                    # If select button is pressed, select a PID for the
                    # specific gadgit.
                        ...
                        # Get a list of all valid PIDs the connected ECU
                        # supports.
                        ValidPIDs = ThisELM327.GetValidPIDs()
                        # Get the information available for each of the
                        # supported PIDs.
                        SelectText = "NONE\n"
                        for PID in sorted(ValidPIDs):
                            if ValidPIDs[PID][ELM327.FIELD_PID_DESCRIPTION] != '!':
                        ...
                        ThisDisplay.CurrentTab["SELECT"] = Select.Select(
                            ThisDisplay.ThisSurface, "SELECT_PID", SelectText)
                    # If close button is pressed, close the relavent dialog.
                        ...
                        if ButtonGadgit["BUTTON"] == "SELECT":
                            ...
                            ThisDisplay.CurrentTab.pop("CONFIGURE", None)
                    # If vehicle button is pressed, get the vehicle data from
                    # the ECU.
                    elif ButtonGadgit["BUTTON"] == "VEHICLE":
                        if LockELM327.acquire(0):
                            _thread.start_new_thread(
                                VehicleData, (ThisDisplay, ))
                    # If trouble or refresh button is pressed, get the trobule
                    # related data from the ECU.
                        ...
                        if LockELM327.acquire(0):
                            ...
                            if ThisELM327.GetMilOn():
                                FlashVisuals["MIL"] = ThisDisplay.Buttons["MIL"]
                    # If clear button is pressed, clear the trouble and related
                    # data on the ECU.
                    ...
                    # If freeze button is pressed.
                        ...
                        if LockELM327.acquire(0):
                            _thread.start_new_thread(
                                FreezeFrameData, (ThisDisplay, ))
                    # If frame button is pressed, get a frame of data from the
                    # ECU.
                    elif ButtonGadgit["BUTTON"] == "FRAME" or ButtonGadgit["BUTTON"] == "RELOAD":
                        if LockELM327.acquire(0):
                            _thread.start_new_thread(
                                FrameData, (ThisDisplay, ))
                    # If add button is pressed, add a new gadgit to the meters
                    # tab.
                    elif ButtonGadgit["BUTTON"] == "ADD":
                        if ThisDisplay.CurrentTab == ThisDisplay.Meters:
                            NewName = "{:X}".format(random.getrandbits(128))
                            ThisDisplay.Meters[NewName] = Gadgit.Gadgit(
                                ThisDisplay.ThisSurface,
                                NewName,
                                Visual.PRESS_NONE,
                                0,
                                2 * Visual.BUTTON_HEIGHT,
                                ThisDisplay.GadgitWidth,
                                ThisDisplay.GadgitHeight,
                                "NEW")
                    ...
                    # tab.
                        ...
                        for ThisGadget in ThisDisplay.Meters:
                            if type(
                                    ThisDisplay.Meters[ThisGadget]) is not str and type(
                                    ThisDisplay.Meters[ThisGadget]) is not Button.Button:
                                for ThisButton in ThisDisplay.Meters[ThisGadget].Buttons:
                                    if ThisDisplay.Meters["LOCK"].GetDown(
                                    ) == False:
                                        ThisDisplay.Meters[ThisGadget].Buttons[ThisButton].SetVisible(
                                            True)
                                    else:
                                        ThisDisplay.Meters[ThisGadget].Buttons[ThisButton].SetVisible(
                                            False)
                ...
                ButtonGadgit = ThisDisplay.IsEvent(
                    Visual.EVENT_MOUSE_UP,
                    ThisEvent.pos[0],
                    ThisEvent.pos[1],
                    ThisEvent.button)
            elif ThisEvent.type == pygame.MOUSEMOTION:
                # When a button is down, pass movement events to all buttons
                # and gadgits.
                if ThisEvent.buttons[0] > 0:
                    ButtonGadgit = ThisDisplay.IsEvent(
                        Visual.EVENT_MOUSE_MOVE,
                        ThisEvent.pos[0],
                        ThisEvent.pos[1],
                        ThisEvent.buttons[0])
                else:
                    ButtonGadgit = ThisDisplay.IsEvent(
                        Visual.EVENT_MOUSE_HOVER,
                        ThisEvent.pos[0],
                        ThisEvent.pos[1],
                        ThisEvent.buttons[0])
